...
char *DS3231::getDOWStr(uint8_t format)
{
	...
	char *daysLong[]  = {"Segunda", "Terca", "Quarta", "Quinta", "Sexta", "Sabado", "Domingo"};
	char *daysShort[] = {"Seg", "Ter", "Qua", "Qui", "Sex", "Sab", "Dom"};
	Time t;
	t=getTime();
	if (format == FORMAT_SHORT)
...
char *DS3231::getMonthStr(uint8_t format)
{
	...
	char *monthLong[]  = {"Janeiro", "Fevereiro", "Marco", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"};
	char *monthShort[] = {"Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"};
	Time t;
	t=getTime();
	if (format == FORMAT_SHORT)
